...
namespace :timun do
  ...
  Cucumber::Rake::Task.new(:test, 'Run Timun Automation Test') do |t|
    # sample use: rake timun:test t=@login REPORT_NAME=2
  ...
  task :parallel do
    ...
    Rake::Task['timun:clear_report'].execute
    Rake::Task['timun:init_report'].execute
    ...
    begin
      ...
      puts '=====::  ::====='
      # p "merging report:"
      # Rake::Task["timun:merge_report"].execute
    end
  end
  ...
  task :merge_report do
    ...
    # sample usage `rake timun:merge_report REPORT_PATH=2018-09-21_14-42-22`
  ...
  task :run do
    ...
    # Test 1
    Rake::Task['timun:init_report'].execute
    system 'rake timun:test t=@login'

    # Test 2
    Rake::Task['timun:init_report'].execute
    system 'rake timun:test t=@signup'
  ...
  task :rerun do
    if File.size("report/#{ENV['REPORT_PATH']}/rerun.txt").zero?
      ...
      system "bundle exec cucumber @rerun.txt --format pretty --format html --out report/#{ENV['REPORT_PATH']}/features_report_rerun.html --format json --out=report/#{ENV['REPORT_PATH']}/cucumber_rerun.json -f rerun  -o report/#{ENV['REPORT_PATH']}/rerun.txt"
    end
  end
